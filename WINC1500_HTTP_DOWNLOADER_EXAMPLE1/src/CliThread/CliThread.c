...
static const CLI_Command_Definition_t xSmokeSensorGetData = {"get_smoke",
															 "get_smoke: Returns the raw smoke sensor data.\r\n",
															 (const pdCOMMAND_LINE_CALLBACK)CLI_GetSmokeSensorData,
														 	 0};	
	
static const CLI_Command_Definition_t xCurrentSensorGetData_1 = {"get_current_1",
																 "get_current_1: Returns the raw current data.\r\n",
																 (const pdCOMMAND_LINE_CALLBACK)CLI_GetCurrentSensorData_1,
																 0};
															   
static const CLI_Command_Definition_t xCurrentSensorGetData_2 = {"get_current_2",
																 "get_current_2: Returns the raw current data.\r\n",
																 (const pdCOMMAND_LINE_CALLBACK)CLI_GetCurrentSensorData_2,
																 0};	
...
static bool init_curr_sensor = false; 
char buffer[100];
uint8_t a_char[6]= {0x00, 0x7C, 0x12, 0x11, 0x12, 0x7C};
/******************************************************************************
 * Callback Functions
 ******************************************************************************/
...
void vCommandConsoleTask(void *pvParameters)
{
	...
	FreeRTOS_CLIRegisterCommand(&xSmokeSensorGetData);
	FreeRTOS_CLIRegisterCommand(&xCurrentSensorGetData_1);
	FreeRTOS_CLIRegisterCommand(&xCurrentSensorGetData_2);
...
{
	char bootloader_file_init_2[] = "0:bootloader.txt";///<Test TEXT File name for bootloader initialization
	const TickType_t xDelay = 20000 / portTICK_PERIOD_MS;
	WifiHandlerSetState(WIFI_DOWNLOAD_INIT);
	f_unlink(bootloader_file_init_2);
	vTaskDelay( xDelay );
	return pdFALSE;
}
...
/**************************************************************************/ /**
 * @brief    Get data from the smoke sensor by accessing the ADC.
 * @param    p_cli 
 * @param    argc 
 * @param    argv 
 ******************************************************************************/
BaseType_t CLI_GetSmokeSensorData(char *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString)
{
	...
	bool smoke_res = (result >= 170) ? true : false;
...
/**************************************************************************/ /**
 * @brief    Get the data from the current sensor by accessing the I2C
 *			 current sensor.
 * @param    p_cli 
 * @param    argc 
 * @param    argv 
 ******************************************************************************/
BaseType_t CLI_GetCurrentSensorData_1(char *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString)
{
	...
	// Create the buffer that should be printed out.
	snprintf(buffer, 100, "Voltage = %d.%d V | Current = %d.%d mA | Power = %d.%d mW\r\n",
	data_bus_volt[0], data_bus_volt[1],
	data_current[0], data_current[1], data_power[0], data_power[1]);
 ...
 * @param    argv 
 ******************************************************************************/
BaseType_t CLI_GetCurrentSensorData_2(char *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString)
{

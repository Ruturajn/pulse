...
/*************************** OUR CODE HERE ***************************/
QueueHandle_t xQueueINA219Sens1 = NULL;
QueueHandle_t xQueueINA219Sens2 = NULL;
QueueHandle_t xQueueSmokeSens = NULL;

bool up_firmware = false;
/*************************** OUR CODE HERE ***************************/
...
static void MQTT_HandleINA1Messages();
static void MQTT_HandleINA2Messages();
static void MQTT_HandleSmokeMessages();
/******************************************************************************
 * Callback Functions
 ******************************************************************************/
...
{	
	...
	{
		SerialConsoleWriteString("[ * ]REMOTE AUTHORIZATION RECEIVED FOR EFUSE 1!!\r\n");
		SerialConsoleWriteString("[ * ]RESTORING SUPPLY!!\r\n\r\n");
...
void SubscribeHandlerEFUSE2Topic(MessageData *msgData)
{
	...
	if (strncmp(msgData->message->payload, "1", 1) ==  0)
	{
		SerialConsoleWriteString("[ * ]REMOTE AUTHORIZATION RECEIVED FOR EFUSE 2!!\r\n");
		SerialConsoleWriteString("[ * ]RESTORING SUPPLY!!\r\n\r\n");
...
void SubscribeHandlerOTAFUTopic(MessageData *msgData)
{
	LogMessage(LOG_DEBUG_LVL, "%.*s\r\n", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data);
	SerialConsoleWriteString("[ * ]REMOTE FIRMWARE UPDATE REQUEST RECEIVED!!\r\n");
	SerialConsoleWriteString("[ * ]UPDATING FIRMWARE!!\r\n\r\n");
	up_firmware = true;
}
/*************************** OUR CODE HERE ***************************/
    ...
    switch (type) {
        ...
        case MQTT_CALLBACK_CONNECTED:
            if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
				...
				/**************************** OUR CODE HERE ****************************/
				mqtt_subscribe(module_inst, EFUSE1_CONTROL, 2, SubscribeHandlerEFUSE1Topic);
				mqtt_subscribe(module_inst, EFUSE2_CONTROL, 2, SubscribeHandlerEFUSE2Topic);
				mqtt_subscribe(module_inst, OTAFU_CONTROL, 2, SubscribeHandlerOTAFUTopic);
				/**************************** OUR CODE HERE ****************************/
...
// 	   SerialConsoleWriteString(mqtt_msg_temp);
//     isPressed = true;
//     //Published in the Wifi thread main loop

	SerialConsoleWriteString("[ * ]ON-BOARD AUTHORIZATION RECEIVED!!\r\n");
	SerialConsoleWriteString("[ * ]RESTORING SUPPLY!!\r\n\r\n"); 
	port_pin_set_output_level(EFUSE_1_PIN, true);
	port_pin_set_output_level(EFUSE_2_PIN, true);
	efuse_stat_1 = true;
...
static void MQTT_HandleTransactions(void)
{
	...
	/*************************** OUR CODE HERE ***************************/
	MQTT_HandleINA1Messages();
	MQTT_HandleINA2Messages();
	MQTT_HandleSmokeMessages();
	/*************************** OUR CODE HERE ***************************/
...
static void MQTT_HandleINA1Messages()
{
	struct INA219Sens1 ina1;
	if (xQueueReceive(xQueueINA219Sens1, &ina1, 0) == pdPASS)
	{
		//SerialConsoleWriteString("In INA1\r\n");
		snprintf(mqtt_msg, 63, "%d.%d", (uint16_t)ina1.current, (uint16_t)((ina1.current - (uint16_t)ina1.current)*1000));
		mqtt_publish(&mqtt_inst, CURR_SENS1_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
			
		snprintf(mqtt_msg, 63, "%d.%d", (uint16_t)ina1.voltage, (uint16_t)((ina1.voltage - (uint16_t)ina1.voltage)*1000));
		mqtt_publish(&mqtt_inst, VOLT_SENS1_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
		
		snprintf(mqtt_msg, 63, "%d", (uint16_t)ina1.comm_status);
		mqtt_publish(&mqtt_inst, SENS1_COMM, mqtt_msg, strlen(mqtt_msg), 1, 0);

		snprintf(mqtt_msg, 63, "%d", (uint16_t)ina1.efuse_1_stat);
		mqtt_publish(&mqtt_inst, EFUSE1_STAT, mqtt_msg, strlen(mqtt_msg), 1, 0);
				
		snprintf(mqtt_msg, 63, "%d.%d", (uint16_t)ina1.power, (uint16_t)((ina1.power - (uint16_t)ina1.power)*1000));
		mqtt_publish(&mqtt_inst, POW_SENS1_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
	}
}

static void MQTT_HandleINA2Messages()
{
	struct INA219Sens2 ina2;
	if (xQueueReceive(xQueueINA219Sens2, &ina2, 0) == pdPASS)
	{
		snprintf(mqtt_msg, 63, "%d.%d", (uint16_t)ina2.current, (uint16_t)((ina2.current - (uint16_t)ina2.current)*1000));
		mqtt_publish(&mqtt_inst, CURR_SENS2_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
		
		snprintf(mqtt_msg, 63, "%d.%d", (uint16_t)ina2.voltage, (uint16_t)((ina2.voltage - (uint16_t)ina2.voltage)*1000));
		mqtt_publish(&mqtt_inst, VOLT_SENS2_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
		
		snprintf(mqtt_msg, 63, "%d", (uint16_t)ina2.comm_status);
		mqtt_publish(&mqtt_inst, SENS2_COMM, mqtt_msg, strlen(mqtt_msg), 1, 0);
		
		snprintf(mqtt_msg, 63, "%d", (uint16_t)ina2.efuse_2_stat);
		mqtt_publish(&mqtt_inst, EFUSE2_STAT, mqtt_msg, strlen(mqtt_msg), 1, 0);
		
		snprintf(mqtt_msg, 63, "%d.%d", (uint16_t)ina2.power, (uint16_t)((ina2.power - (uint16_t)ina2.power)*1000));
		mqtt_publish(&mqtt_inst, POW_SENS2_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
	}	
}
...
{
	bool smoke_val = 0;
	if (xQueueReceive(xQueueSmokeSens, &smoke_val, 0) == pdPASS)
	{
		snprintf(mqtt_msg, 63, "%d", smoke_val);
		mqtt_publish(&mqtt_inst, SMOKE_SENS_VAL, mqtt_msg, strlen(mqtt_msg), 1, 0);
	}
}
...
void vWifiTask(void *pvParameters)
{
	...
	/*************************** OUR CODE HERE ***************************/
	xQueueINA219Sens1 = xQueueCreate(5, sizeof(struct INA219Sens1));
	xQueueINA219Sens2 = xQueueCreate(5, sizeof(struct INA219Sens2));
	xQueueSmokeSens = xQueueCreate(5, sizeof(bool));
	/*************************** OUR CODE HERE ***************************/
...
int WifiAddSmokeDataToQueue(bool *smoke_val)
{
    int error = xQueueSend(xQueueSmokeSens, smoke_val, (TickType_t)10);
    return error;	
}

int WifiAddINA219OneDataToQueue(struct INA219Sens1 *ina)
{
	int error = xQueueSend(xQueueINA219Sens1, ina, (TickType_t)10);
	//SerialConsoleWriteString("In adding INA1\r\n");
	return error;
}

int WifiAddINA219TwoDataToQueue(struct INA219Sens2 *ina)
{
	int error = xQueueSend(xQueueINA219Sens2, ina, (TickType_t)10);
	return error;
}
